...
for t=1:maxit
    ...
    %symmetry u
    for j=1:ny+2
        u_k_minus_1(1,j)=0.0; u_k_minus_1(nx+1,j)=0.0; %side walls
    end
    %symmetry v
    for j=2:ny
        v_k_minus_1(1,j)=v_k_minus_1(2,j);%bottom wall
        v_k_minus_1(nx+2,j)=v_k_minus_1(nx+1,j);%top wall
    end
    %traction free v
    for i=1:nx+2
        v_k_minus_1(i,1)=0.0; v_k_minus_1(i,ny+1)=0.0;%side wall
    end
    %traction free p
    for i=1:nx+2
    ...
    %symmetry p
    for j=2:ny+1
        p_dash(nx+2,j)=p_dash(nx+1,j); p_dash(1,j)=p_dash(2,j);%side wall
    end
    %constant wall temperature
    for i=2:nx+1
        if(i>0 && i<=round(0.2/dx))
            T_k_minus_1(i,1)=2-T_k_minus_1(i,2);%bottom wall
        end
        if(i>round(0.2/dx) && i<=round(0.4/dx))
            T_k_minus_1(i,1)=T_k_minus_1(i,2);%bottom wall
        end
        if(i>round(0.4/dx) && i<=round(0.6/dx))
            T_k_minus_1(i,1)=2-T_k_minus_1(i,2);%bottom wall
        end
        if(i>round(0.6/dx) && i<=round(0.8/dx))
            T_k_minus_1(i,1)=T_k_minus_1(i,2);%bottom wall
        end
        if(i>round(0.8/dx) && i<=nx+1)
            T_k_minus_1(i,1)=2-T_k_minus_1(i,2);%bottom wall
        end
    ...
    for j=1:ny+2
        T_k_minus_1(1,j)=T_k_minus_1(2,j);T_k_minus_1(nx+2,j)=T_k_minus_1(nx+1,j);%side walls
    end
    ...
    for j=1:ny+2
        C_k_minus_1(1,j)=C_k_minus_1(2,j); C_k_minus_1(nx+2,j)=C_k_minus_1(nx+1,j);%side walls
    end
end
