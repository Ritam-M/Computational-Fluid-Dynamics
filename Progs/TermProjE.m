...
for t=1:maxit
    ...
    %pressure updation (p_new=p_star+alpha_P*p_dash, to reduce variables p_new and p_dash are taken as the same).
    for i=2:nx+1
        for j=2:ny+1
    ...
    %boundary conditions :
    for i=2:nx
        u_k_minus_1(i,1)=u_k_minus_1(i,2); u_k_minus_1(i,ny+2)=u_k_minus_1(i,ny+1);
    end
    for j=1:ny+2
        u_k_minus_1(1,j)=0.0; u_k_minus_1(nx+1,j)=0.0;
    end
    for j=2:ny
        v_k_minus_1(1,j)=v_k_minus_1(2,j); v_k_minus_1(nx+2,j)=v_k_minus_1(nx+1,j);
    end
    for i=1:nx+2
        v_k_minus_1(i,1)=0.0; v_k_minus_1(i,ny+1)=0.0;
    end
    for i=1:nx+2
        p_dash(i,1)=-p_dash(i,2); p_dash(i,ny+2)=-p_dash(i,ny+1);
    end
    for j=2:ny+1
        p_dash(nx+2,j)=p_dash(nx+1,j); p_dash(1,j)=p_dash(2,j);
    end
    for i=2:nx+1
        T_k_minus_1(i,1)=2-T_k_minus_1(i,2); T_k_minus_1(i,ny+2)=-T_k_minus_1(i,ny+1); 
    end
    for j=1:ny+2
        T_k_minus_1(1,j)=T_k_minus_1(2,j); T_k_minus_1(nx+2,j)=T_k_minus_1(nx+1,j);
    end
    for i=2:nx+1
        C_k_minus_1(i,1)=2-C_k_minus_1(i,2); C_k_minus_1(i,ny+2)=-C_k_minus_1(i,ny+1); 
    end
    for j=1:ny+2
        C_k_minus_1(1,j)=C_k_minus_1(2,j); C_k_minus_1(nx+2,j)=C_k_minus_1(nx+1,j);
    end
end
...
for i=1:nx+1
    for j=1:ny+1
        u_cell(i,j)=(u_k_minus_1(i,j+1)+u_k_minus_1(i,j))/2;
        v_cell(i,j)=(v_k_minus_1(i+1,j)+v_k_minus_1(i,j))/2;
        p_cell(i,j)=(p_dash(i,j)+p_dash(i+1,j)+p_dash(i+1,j+1)+p_dash(i,j+1))/4;
        T_cell(i,j)=(T_k_minus_1(i,j)+T_k_minus_1(i+1,j)+T_k_minus_1(i+1,j+1)+T_k_minus_1(i,j+1))/4;
        C_cell(i,j)=(C_k_minus_1(i,j)+C_k_minus_1(i+1,j)+C_k_minus_1(i+1,j+1)+C_k_minus_1(i,j+1))/4;
    end
end
...
F=figure(1);
subplot(1,3,1);
[c1,h1]=contour(linspace(0,1,nx+1),linspace(0,1,ny+1),psi_new.',15);
set(h1,'edgecolor','k');
title('Streamline');
subplot(1,3,2);
cMap=jet(256);
[c2,h2]=contourf(linspace(0,1,nx+1),linspace(0,1,ny+1),T_cell.',100);
set(h2,'edgecolor','none');
colormap(cMap);
title('Isotherm');
subplot(1,3,3);
cMap=jet(256);
[c3,h3]=contourf(linspace(0,1,nx+1),linspace(0,1,ny+1),C_cell.',100);
set(h3,'edgecolor','none');
colormap(cMap);
title('Iso-concentration');
set(F,'WindowStyle','docked');

x=linspace(0,1,nx+1); y=linspace(0,1,ny+1); data=zeros((nx+1)*(ny+1),9); k=1;
for i=1:nx+1
    for j=1:ny+1
